...
impl<'a> LopdfTocAdapter<'a> {
    ...
    fn doc_to_bytes(mut doc: Document) -> Result<Vec<u8>> {
        ...
        doc.save_to(&mut buffer)
            .map_err(|e| format_err!("Lopdf save error: {}", e))?;
        Ok(buffer)
    }
}
...
impl<'a> TocEditor for LopdfTocAdapter<'a> {
    ...
    fn inject_links(
        ...
        insert_pos: usize,
        original_page_ids: &[String],
    ) -> Result<Vec<u8>> {
        let mut doc =
            Document::load_mem(pdf_bytes).map_err(|e| format_err!("Lopdf reload error: {}", e))?;
        ...
        for link in links {
            ...
            if toc_page_idx_in_merged >= merged_pages_list.len() {
                continue;
            }
            ...
            let target_page_id = if link.target_is_original_doc {
                if target_idx >= original_page_ids.len() {
                    continue;
                }
                Self::parse_id(&original_page_ids[target_idx])?
            } else {
                if target_idx >= merged_pages_list.len() {
                    continue;
                }
            ...
            let page_height = {
                ...
                let box_array = page_dict
                    ...
                    .map(|a| {
                        a.iter()
                            .filter_map(|n| n.as_float().ok().map(|f| f as f64))
                            .collect::<Vec<f64>>()
                    });
    ...
    fn apply_page_labels(
        &mut self,
        pdf_bytes: &[u8],
        toc_pdf_path: &str,
        insert_pos: i32,
        toc_label: Option<&PageLabel>,
    ) -> Result<Vec<u8>> {
        let mut doc =
            Document::load_mem(pdf_bytes).map_err(|e| format_err!("Lopdf reload error: {}", e))?;

        let src_adapter = crate::pdf::lopdf::page_label_adapter::LopdfPageLabelAdapter::new(
            self.session.get_lopdf_doc_mut()?,
        );
        ...
        if let Ok(rules) = src_adapter.get_label_rules() {
            let (toc_len, toc_rules) = if let Ok(mut toc_doc) = Document::load(toc_pdf_path) {
                ...
                let toc_adapter =
                    crate::pdf::lopdf::page_label_adapter::LopdfPageLabelAdapter::new(&mut toc_doc);
            ...
            if toc_len > 0 {
                let new_rules = crate::pdf::toc::calculate_merged_rules(
                    rules, insert_pos, toc_len, toc_rules, toc_label,
                );
                let mut dest_adapter =
                    crate::pdf::lopdf::page_label_adapter::LopdfPageLabelAdapter::new(&mut doc);
