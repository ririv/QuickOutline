...
pub fn internal_render_page(
    state: &mut PdfWorkerInternalState,
    path: String,
    page_index: u16,
    scale: f32,
) -> Result<Vec<u8>> {
    ...
    if let Some(cached) = session.render_cache.get(&(page_index, scale_key.clone())) {
        info!(
            "[PdfRender] Cache hit for page {} scale {}",
            page_index, scale
        );
    ...
    let doc = session
        .pdfium_doc
        .as_ref()
        .ok_or_else(|| format_err!("Session missing document"))?;
    ...
    session
        .render_cache
        .put((page_index, scale_key), png_data.clone());
...
pub async fn render_pdf_page(
    ...
    state
        .call(move |worker| internal_render_page(worker, path, page_index, scale))
        .await
...
#[tauri::command]
pub async fn get_pdf_page_count(state: State<'_, PdfWorker>, path: String) -> Result<u16, String> {
    state
        .call(move |worker| internal_get_page_count(worker, path))
        .await
        .map_err(|e| e.to_string())?
        .map_err(|e| e.to_string())
}
