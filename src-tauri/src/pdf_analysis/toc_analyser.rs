...
static TOC_DOT_PATTERN: Lazy<Regex> =
    Lazy::new(|| Regex::new(r".*([.]\s*|\s{2,}){4,}\s*\d+\s*$").expect("Invalid TOC_DOT_PATTERN"));
#[allow(clippy::expect_used)]
static TOC_NUMERIC_END_PATTERN: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"^(.*[^0-9])\s+([0-9]+)\s*$").expect("Invalid TOC_NUMERIC_END_PATTERN")
});
...
impl TocAnalyser {
    pub fn find_dominant_style(all_blocks: &Vec<PdfBlock>) -> PdfStyle {
        if all_blocks.is_empty() {
            return PdfStyle::new("Default".to_string(), 10.0);
        }
    ...
    pub fn find_toc_blocks_in_page(
        page_blocks: &mut Vec<PdfBlock>,
        dominant_style: &PdfStyle,
    ) -> Vec<PdfBlock> {
    ...
    fn is_toc_like_block(block: &mut PdfBlock, dominant_style: &PdfStyle) -> bool {
        ...
        if !(3..=150).contains(&char_count) {
            return false;
        }

        if TOC_DOT_PATTERN.is_match(text) {
            return true;
        }
