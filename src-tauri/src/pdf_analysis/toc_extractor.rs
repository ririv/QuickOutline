...
impl TocExtractor {
    pub fn extract_toc<D: PdfDocumentTrait + ?Sized>(doc: &D) -> Result<Vec<String>> {
        ...
        info!(
            "[TocExtractor] Processing {} chunks serially...",
            chunks.len()
        );
            ...
            for (start, end) in chunks {
                ...
                for i in start..end {
                    if let Ok(page) = doc.get_page(i) {
                        ...
                        let page_blocks =
                            PdfProcessor::extract_blocks_from_page(page.as_ref(), page_num);
        ...
        info!(
            "[TocExtractor] Total raw blocks extracted: {}",
            all_blocks.len()
        );
        ...
        info!(
            "[TocExtractor] Dominant style: {:?} (Size: {})",
            dominant_style.font_name,
            dominant_style.get_size()
        );

        let mut blocks_by_page: BTreeMap<i32, Vec<PdfBlock>> = BTreeMap::new();
        for block in all_blocks {
            blocks_by_page
                .entry(block.get_page_num())
                .or_default()
                .push(block);
        }
        ...
        let toc_results: Vec<Vec<String>> = pages_blocks
            .into_par_iter()
            .map(|mut page_blocks| {
                let filtered =
                    TocAnalyser::find_toc_blocks_in_page(&mut page_blocks, &dominant_style);
                filtered
                    .into_iter()
