...
fn main() -> Result<()> {
    ...
    let (file_name, binary_name, inner_path) = match (target_os.as_str(), target_arch.as_str()) {
        ...
        ("windows", "x86_64") => ("pdfium-win-x64.tgz", "pdfium.dll", "bin/pdfium.dll"),
        ("windows", "aarch64") => ("pdfium-win-arm64.tgz", "pdfium.dll", "bin/pdfium.dll"),
        ("linux", "x86_64") => ("pdfium-linux-x64.tgz", "libpdfium.so", "lib/libpdfium.so"),
        _ => {
            println!(
                "cargo:warning=Unsupported platform for pdfium download: {}-{}",
    ...
    if !dest_path.exists() {
        ...
        println!(
            "cargo:warning=PDFium library already exists at {:?}",
            dest_path
        );
    }
