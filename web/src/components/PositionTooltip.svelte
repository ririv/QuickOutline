<script lang="ts">
  ...
  const color = pos === 'inner' ? '#ff4d4f' : '#1677ff';
  
  // Determine title
  const title = pos === 'inner' ? 'Inner (Binding Side)' : 'Outer (Edge Side)';
  ...
  if (pos === 'inner') {
      ...
      lRect.x = 58 - hWidth; // 10 + 48 - width
      rRect.x = 62; // 62
  } else {
      // Outer: Left Page Left Side, Right Page Right Side
      lRect.x = 10; // 10
      rRect.x = 110 - hWidth; // 62 + 48 - width
  }
</script>
